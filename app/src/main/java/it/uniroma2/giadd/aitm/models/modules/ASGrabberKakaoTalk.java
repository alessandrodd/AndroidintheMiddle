...
public class ASGrabberKakaoTalk implements AutonomousSystemGrabber {
    ...
    public void getRoutingPrefixes(final Context context, final OnAutonomousSystemGrabbedListener listener) {
        Ion.with(context)
                .load(STAT_RIPE_ANNOUNCED_PREFIXES_BY_AS_URI + KAKAOTALK_AS1).as(RipeAnnouncedPrefix.class)
                .setCallback(new FutureCallback<RipeAnnouncedPrefix>() {
                    ...
                    public void onCompleted(Exception e, RipeAnnouncedPrefix result) {
                        ...
                        final ArrayList<String> stringPrefixes = new ArrayList<String>();
                        for (Prefix prefix : prefixes) {
                            stringPrefixes.add(prefix.getPrefix());
                        }
                        Ion.with(context)
                                .load(STAT_RIPE_ANNOUNCED_PREFIXES_BY_AS_URI + KAKAOTALK_AS2).as(RipeAnnouncedPrefix.class)
                                .setCallback(new FutureCallback<RipeAnnouncedPrefix>() {
                                    @Override
                                    public void onCompleted(Exception e, RipeAnnouncedPrefix result) {
                                        if (e != null) {
