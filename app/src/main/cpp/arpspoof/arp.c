...
int arp_lookup(in_addr_t address, struct ether_addr *ether, const char *ifname) {
    ...
    while (fgets(line, sizeof(line), fp)) {
         ...
         */
        if (strcmp(hwa, "00:00:00:00:00:00") == 0 || strcmp(hwa, "FF:FF:FF:FF:FF:FF") == 0 ||
        ...
        if (strcmp(ip, ipaddress) == 0) {
            // printf("Mac address found for ip %s ; mac is %s\n", ip, hwa);
...
int arp_send(libnet_t *pnet, int opcode, unsigned char *source_hardware, in_addr_t source_address,
             unsigned char *target_hardware, in_addr_t target_address) {
    int retval = -1;

    if (source_hardware != NULL ||
        (source_hardware = (unsigned char *) libnet_get_hwaddr(pnet)) != NULL) {
        if (source_address != 0 || (source_address = libnet_get_ipaddr4(pnet)) != -1) {
            if (target_hardware == NULL)
                target_hardware = BROADCAST_MAC_ADDR;

            if (libnet_autobuild_arp(opcode, source_hardware, (unsigned char *) &source_address,
                                     target_hardware, (unsigned char *) &target_address, pnet) !=
                -1) {
                if (libnet_build_ethernet(target_hardware, source_hardware, ETHERTYPE_ARP, NULL, 0,
                                          pnet, 0) != -1) {
                    ...
                    if (retval == -1)
                        ...
                        printf
                                (
                                        ...
                                        source_hardware[0], source_hardware[1], source_hardware[2],
                                        source_hardware[3], source_hardware[4], source_hardware[5],
                                        inet_ntoa(*(struct in_addr *) &target_address),
                                        target_hardware[0], target_hardware[1], target_hardware[2],
                                        target_hardware[3], target_hardware[4], target_hardware[5]
                                );
